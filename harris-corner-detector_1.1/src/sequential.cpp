...
)
{
  if (argc < 2){
    ...
    char fullpath[1024];
    // specify the directory path for data images
    ...
    //check if the directory was opened succesfully
    if (dir == NULL){
        printf("bad");
...
{      
  ...
  int result=read_parameters(
    ...
    char *images[] = {"data/moutains.jpg", "data/building.png", "data/calibrator.png", "data/nature.jpg"};
    for (int i=0; i<4; i++) {
        image = images[i];
  if(result)
  {
   ...
   gettimeofday(&endT1, NULL);
    printf("\nTime T1: %fs\n", ((endT1.tv_sec-startT1.tv_sec)* 1000000u + 
            endT1.tv_usec - startT1.tv_usec) / 1.e6);
