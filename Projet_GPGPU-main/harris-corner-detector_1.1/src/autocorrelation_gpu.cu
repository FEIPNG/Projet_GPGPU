...
__global__ void compute_autocorrelation_matrix_kernel_texture(float *A, float *B, float *C, int nx, int ny) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    int p = x + y * blockDim.x * gridDim.x;

    float Ix_value = tex1D(texIx, x);  // Read from texture memory
    float Iy_value = tex1D(texIy, x);
...
void compute_autocorrelation_matrix_cuda(float *Ix, float *Iy, float *A, float *B, float *C, int nx, int ny){
    ...
    cudaMallocArray(&d_IxArray, &texIx.channelDesc, nx, 1);
    cudaMallocArray(&d_IyArray, &texIy.channelDesc, nx, 1);
    ...
    compute_autocorrelation_matrix_kernel_texture<<<gridSize, blockSize>>>(d_A, d_B, d_C, nx, ny);
